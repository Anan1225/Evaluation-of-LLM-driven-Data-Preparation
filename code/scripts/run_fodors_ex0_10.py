...
def build_specs(model: str, seed: int, repeat: int) -> list[EMRunSpec]:
    ...
    for example_num in range(0, 11):
        ...
        specs.append(
            EMRunSpec(
                run_id=run_id,
                sweep_axis="fodors-zagats-example-num",
                p=0.5,
                r_len=1.0,
                k=3,
                example_num=example_num,
...
def patch_cfg_for_fodors_zagats(cfg: dict, out_dir: str) -> dict:
    ...
    # Fodors-Zagats test.csv already includes labels.
    em["test_label_csv"] = "./datasets/entity_matching/Fodors-Zagats/test.csv"

    patched.setdefault("em_sweep", {})
    patched["em_sweep"]["key_attributes"] = ["name", "addr", "city", "phone", "type", "class"]
    patched["out_dir"] = out_dir
    return patched


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Fodors-Zagats EM for example_num=0..10 at fixed p=0.5, r_len=1.0, k=3")
    parser.add_argument("--model", default="gpt52", help="Model alias or raw model name (default: gpt52)")
    ...
    batch_dir = (
        Path(args.out_dir)
        / f"batch_fodors_zagats_p50_r100_k3_seed{args.seed}_rep{args.repeat}_{stamp}"
    )
    cfg = patch_cfg_for_fodors_zagats(cfg, out_dir=str(batch_dir))
    ...
    # Also write a dedicated summary for this scripted batch.
    dedicated = summary_dir / f"summary_fodors_zagats_p50_r100_k3_seed{args.seed}_rep{args.repeat}.csv"
