...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Fodors-Zagats EM (gemini pro) for example_num=0..10 at fixed p=0.5, r_len=1.0, k=3")
    ...
    batch_dir = Path(args.out_dir) / f"batch_fodors_zagats_gemini_pro_p50_r100_k3_seed{args.seed}_rep{args.repeat}_{stamp}"
    cfg = patch_cfg_for_fodors_zagats(cfg, out_dir=str(batch_dir))

    provider, model, api_key = run.map_model("gp", cfg)
    ...
    dedicated = summary_dir / f"summary_fodors_zagats_gemini_pro_p50_r100_k3_seed{args.seed}_rep{args.repeat}.csv"
